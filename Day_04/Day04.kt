...
fun main() {
    ...
    val charCoordinates = mapOf(
        'X' to HashSet<Pair<Int, Int>>(),
        'M' to HashSet<Pair<Int, Int>>(),
        'A' to HashSet<Pair<Int, Int>>(),
        'S' to HashSet<Pair<Int, Int>>()
    )

    // read the coordinates of the relevant letters
    File("Day_04/input.txt").readLines().forEachIndexed { row, line ->
        line.toCharArray().forEachIndexed { column, ch -> charCoordinates[ch]?.add(row to column) }
    }
    ...
    charCoordinates['A']?.sumOf { (aRow, aColumn) ->
        ...
        arrayOf(
            ...
            .count { it in charCoordinates.getOrDefault('S', HashSet()) } / 2
